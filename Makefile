...
MODULE  := github.com/mwirges/ghistx
BINARY  := ghistx
CMD     := ./cmd/ghistx
...
## build: Compile for the current OS/arch → build/ghistx[.exe]
.PHONY: build
build:
	@mkdir -p $(BUILD_DIR)
...
## build-all: Cross-compile for all supported platforms → build/ghistx-{os}-{arch}[.exe]
.PHONY: build-all
